...
struct MobileSyncSummary {
    ...
    static func make(connection: AppStore.ConnectionState, paired: Bool, cachedGames: Int, pending: Int, lastSuccess: Date?) -> Self {
        ...
        switch connection {
        case .online:
            return .init(title: pending == 0 ? "Tudo sincronizado" : "\(pending) alteraç\(pending == 1 ? "ão" : "ões") aguardando o computador",
        ...
        case .offline:
            return .init(title: "Dados salvos no aparelho", detail: "\(pending) alteraç\(pending == 1 ? "ão pendente" : "ões pendentes") · última sincronização \(age)", action: "Reconexão automática ativa", isHealthy: cachedGames > 0)
