...
A versão pessoal para iPhone é compilada separadamente em **Actions → Build iOS pessoal**. O artefato contém um IPA sem assinatura para instalação com AltStore ou Sideloadly. No iOS 0.7.3, `CLASSICS Everywhere` cria uma cópia temporária legível somente na primeira entrega. Depois disso, formatos reconhecidos abrem diretamente o arquivo já importado no RetroArch da App Store com o núcleo correspondente. Consulte [as instruções do iOS](ios/README-IOS.md) e o [guia do RetroArch compatível](ios/RETROARCH-COMPATIVEL.md).
